...
CONF_SEED_CONSTANT = "seed_constant"
...
CONFIG_SCHEMA = cover.cover_schema(EleroCover).extend(
    {
        ...
        # Per-blind encryption constant C for len=27 (UniTec) remotes. Default is the
        # author's value; recover yours with tools/recover_seed_constant.py if a blind
        # receives fine but ignores transmits.
        cv.Optional(CONF_SEED_CONSTANT, default=0x4751): cv.hex_int_range(min=0x0, max=0xffff),
    }
...
async def to_code(config):
    ...
    cg.add(var.set_seed_constant(config[CONF_SEED_CONSTANT]))
