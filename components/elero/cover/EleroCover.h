...
class EleroCover : public cover::Cover, public Component {
 public:
  ...
  // Keep our TX rolling counter just ahead of the real remote's last-seen value.
  void sync_counter(uint8_t cnt) { this->command_.counter = (cnt >= 0xff) ? 1 : (uint8_t)(cnt + 1); }
  void set_supports_tilt(bool tilt) { this->supports_tilt_ = tilt; }
  void set_seed_constant(uint16_t s) { this->command_.seed_const = s; }
 ...
 protected:
  ...
  t_elero_command command_ = {
    .counter = 1,
    .seed_const = 0x4751,   // default; override per-blind via the seed_constant option
  };
  Elero *parent_;
