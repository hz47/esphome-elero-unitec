...
void Elero::loop() {
  // PATCH: GDO0 interrupt doesn't fire reliably on all modules, so poll the RX
  // FIFO. Only read when a complete, sane-sized packet is sitting still in the
  // FIFO; flush noise/overflow. (CRC_AUTOFLUSH in PKTCTRL1 drops bad-CRC frames.)
  static uint32_t last_poll = 0;
  static uint32_t last_restart = 0;
  static uint8_t last_cnt = 0;
  uint32_t now = millis();
  // PATCH: fully ignore the GDO0 "received_" flag. On this module GDO0 throws FALSE
  // triggers (noise), which pinned do_read=true and made the loop flush the FIFO on
  // every cycle - so real packets were wiped before they finished arriving. Drive the
  // receiver purely from RXBYTES polling below.
  this->received_ = false;
  bool do_read = false;

  if (!do_read && (now - last_poll >= 5)) {
    last_poll = now;
    uint8_t rb = this->read_status(CC1101_RXBYTES);
    uint8_t cnt = rb & 0x7F;
    if ((rb & 0x80) || cnt > 62) {           // overflow / impossible length -> noise
      // Non-blocking flush (don't wait_idle - in a noisy band it never settles).
      this->write_cmd(CC1101_SIDLE);
      this->write_cmd(CC1101_SFRX);
      this->write_cmd(CC1101_SRX);
      last_restart = now;
      cnt = 0;
    } else if (cnt >= 26 && cnt == last_cnt) { // stable frame (>=26B); read first packet,
                                               // even if a burst stacked several together
      do_read = true;
    }
    last_cnt = cnt;
  }

  // Keep the receiver alive: clone CC1101s wedge in an RX sub-state if left idle.
  // Periodically re-enter RX (like the working sweep does) when the FIFO is empty.
  if (!do_read && last_cnt == 0 && (now - last_restart >= 90)) {
    last_restart = now;
    this->write_cmd(CC1101_SIDLE);
    this->write_cmd(CC1101_SFRX);
    this->write_cmd(CC1101_SRX);
  ...
  if(do_read) {
    ...
    // Non-blocking flush + restart RX (never wait_idle in a noisy band).
    this->write_cmd(CC1101_SIDLE);
    this->write_cmd(CC1101_SFRX);
    this->write_cmd(CC1101_SRX);
    last_cnt = 0;
...
void Elero::dump_config() {
  ESP_LOGCONFIG(TAG, "Elero Config (patched: CCA disabled, TXBYTES-polled TX done)");
}
...
void Elero::init() {
  ...
  // PATCH: MCSM1 was 0x3F (CCA enabled - only transmit if channel clear).
  // In busy 868 MHz environments this blocks TX ("Timed out waiting for TX").
  // 0x0F disables CCA so STX always transmits. RXOFF/TXOFF still return to RX.
  this->write_reg(CC1101_MCSM1, 0x0F);
...
bool Elero::wait_tx_done() {
  ...
  // PATCH: original code waited on the shared GDO0 "received_" flag, which in a
  // busy 868 MHz environment gets falsely set by INCOMING packets - aborting our
  // transmit early ("Error transferring, N bytes left in buffer").
  // Instead, poll TXBYTES until the TX FIFO has actually drained.
  uint16_t timeout = 1000;
  while (((this->read_status(CC1101_TXBYTES) & 0x7f) != 0) && (--timeout != 0)) {
    delay_microseconds_safe(200);
  }
  // Drop any stray RX flag that arrived while we were transmitting.
  this->received_ = false;
...
bool Elero::transmit() {
  ...
  // BLIND TRANSMIT + FREQ SWEEP: never read status (MISO can stick at 0x0f).
  // Send the same packet across the whole freq0 range so the blind hears whichever
  // setting lands on 868.3 MHz. Once we know which one works we lock it in.
  // Single clean transmit at the configured frequency (MOSI is connected now, so
  // the FIFO write and config are reliable). Blind = no status reads needed.
  this->write_cmd(CC1101_SIDLE);
  delay_microseconds_safe(300);
  this->write_cmd(CC1101_SFTX);
  delay_microseconds_safe(60);
  ...
  this->write_cmd(CC1101_SFRX);
  this->write_cmd(CC1101_SRX);
  ESP_LOGD(TAG, "transmit: sent (blind)");
  return true;
}
...
uint8_t Elero::read_status(uint8_t addr) {
  // PATCH: CC1101 errata - status registers (MARCSTATE, RXBYTES, RSSI, ...) can
  // return a corrupt value on a single read. Read until two reads agree.
  uint8_t d1 = 0, d2 = 0;
  uint8_t tries = 0;
  do {
    this->enable();
...
void Elero::interpret_msg() {
  ...
  // RAW on-air dump (BEFORE decryption) so we can byte-compare against what we transmit.
  {
    char hb[200]; int p = 0;
    uint8_t n = length + 1; if(n > CC1101_FIFO_LENGTH) n = CC1101_FIFO_LENGTH;
    for(uint8_t i = 0; i < n; i++) p += snprintf(hb + p, sizeof(hb) - p, "%02x ", this->msg_rx_[i]);
    ESP_LOGD(TAG, "RAW RX: %s", hb);
  }
  ...
  // ROLLING-CODE SYNC: when we hear the real remote transmit DIRECTLY (src==bwd, i.e.
  // not a relayed/mesh hop), remember its counter so our own transmits go out one step
  // AHEAD of it. The blind rejects packets whose counter is behind the last it saw.
  if(crc && (src == bwd)) {
    for(auto &kv : this->address_to_cover_mapping_) {
      if(kv.second->get_remote_address() == src) {
        kv.second->sync_counter(cnt);
        ESP_LOGD(TAG, "rolling-code: synced our TX counter to %d (remote=0x%06x)", (cnt >= 0xff) ? 1 : cnt + 1, src);
        break;
      }
    }
  }
...
bool Elero::send_command(t_elero_command *cmd) {
  ...
  if(cmd->pck_inf[0] > 0x60) {
    // ---- Original format: len=29, 3-byte destination address (e.g. typ 0x6a) ----
    this->msg_tx_[0] = 0x1d; // message length (29)
    ...
    this->msg_tx_[17] = ((cmd->blind_addr >> 16) & 0xff); // blind address (3 bytes)
    ...
    // ---- PATCH: short format len=27, 1-byte destination address (e.g. typ 0x45) ----
    // Mirrors the receive logic in interpret_msg() for typ <= 0x60.
    this->msg_tx_[0] = 0x1b; // message length (27)
    this->msg_tx_[1] = cmd->counter; // message counter
    this->msg_tx_[2] = cmd->pck_inf[0];
    this->msg_tx_[3] = cmd->pck_inf[1];
    this->msg_tx_[4] = cmd->hop; // hop info
    ...
    this->msg_tx_[15] =((cmd->remote_addr) & 0xff);
    this->msg_tx_[16] = 0x01; // destination count
    this->msg_tx_[17] = ((cmd->blind_addr) & 0xff); // blind address (1 byte)
    for(int i=0; i<10; i++)
      this->msg_tx_[18 + i] = cmd->payload[i];
    // PATCH: the encryption seed is COUNTER-DERIVED (same shape as the len=29 path,
    // but with this blind/remote's system constant C). Verified offline by decoding
    // many real remote packets: the seed recovered during decryption equals
    // (C - cnt*0x708f) & 0xffff for every counter, and re-encoding with this seed
    // reproduces the remote's exact ciphertext. A zero seed (the old code) left the
    // payload unscrambled, so the blind rejected it. C is per-blind: default 0x4751,
    // override via the cover's `seed_constant:` option (recover_seed_constant.py).
    uint16_t code27 = (cmd->seed_const - (cmd->counter * 0x708f)) & 0xffff;
    this->msg_tx_[20] = (code27 >> 8) & 0xff;
    this->msg_tx_[21] = code27 & 0xff;
    uint8_t *payload = &this->msg_tx_[20];
    msg_encode(payload);
  }

  // RAW dump of exactly what we put on air - compare these bytes against "RAW RX".
  {
    char hb[200]; int p = 0;
    uint8_t n = this->msg_tx_[0] + 1; if(n > CC1101_FIFO_LENGTH) n = CC1101_FIFO_LENGTH;
    for(uint8_t i = 0; i < n; i++) p += snprintf(hb + p, sizeof(hb) - p, "%02x ", this->msg_tx_[i]);
    ESP_LOGD(TAG, "RAW TX: %s", hb);
  }

  ESP_LOGV(TAG, "send: len=%02d, cnt=%02d, typ=0x%02x, typ2=0x%02x, hop=0x%02x, syst=0x%02x, chl=%02d, src=0x%02x%02x%02x, payload=[0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x]", this->msg_tx_[0], this->msg_tx_[1], this->msg_tx_[2], this->msg_tx_[3], this->msg_tx_[4], this->msg_tx_[5], this->msg_tx_[6], this->msg_tx_[7], this->msg_tx_[8], this->msg_tx_[9], this->msg_tx_[18], this->msg_tx_[19], this->msg_tx_[20], this->msg_tx_[21], this->msg_tx_[22], this->msg_tx_[23], this->msg_tx_[24], this->msg_tx_[25], this->msg_tx_[26], this->msg_tx_[27]);
  return transmit();
}
