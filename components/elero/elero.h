...
typedef struct {
  ...
  uint8_t hop;
  uint8_t payload[10];
  uint16_t seed_const;   // per-blind encryption constant C for the len=27 seed
} t_elero_command;

class EleroCover;

// PATCH: SPI dropped from 2 MHz to 200 kHz. Some clone CC1101 modules return
// garbage (a stuck 0x0F pattern) on multi-byte burst FIFO reads at 2 MHz because
// MISO is sampled before the chip drives the next bit. Slow, reliable reads fix it.
